...
impl<R: Read> Deserializer<R> {
        ...
        loop {
            match self.read_byte()? {
                ...
                BUILD => {
                    ...
                    match (state, inst) {
                        (Value::Dict(ref r), Value::Dict(ref l)) => {
                            self.stack
                                .push(Value::Dict(l.iter().chain(r).map(|x| x.clone()).collect()));
                        }
                        (state, _) => {
                ...
                PERSID => {
                    ...
                    self.stack
                        .push(Value::PersistentId(Box::new(Value::String(pid))));
                }
                BINPERSID => {
                    let value = self.pop()?;
    ...
    fn decode_global(&mut self, modname: Vec<u8>, globname: Vec<u8>) -> Result<Value> {
        let value = match (&*modname, &*globname) {
            ...
            (b"collections", b"OrderedDict") => Value::Global(Global::OrderedDict),
            (b"torch", b"HalfStorage") => Value::Global(Global::HalfStorage),
            (b"torch", b"FloatStorage") => Value::Global(Global::FloatStorage),
            (b"torch", b"LongStorage") => Value::Global(Global::LongStorage),
            (b"torch._utils", b"_rebuild_tensor_v2") => Value::Global(Global::RebuildTensor),
    ...
    fn convert_value(&mut self, value: Value) -> Result<value::Value> {
        match value {
            ...
            Value::Global(Global::HalfStorage) => Ok(value::Value::String("half".into())),
            Value::Global(Global::FloatStorage) => Ok(value::Value::String("float".into())),
            Value::Global(Global::LongStorage) => Ok(value::Value::String("long".into())),
            Value::Global(_) => {
            ...
            Value::PersistentId(id) => Ok(value::Value::PersistentId(Box::new(
                self.convert_value(*id)?,
            ))),
        }
    }
}
...
impl<'de: 'a, 'a, R: Read> de::Deserializer<'de> for &'a mut Deserializer<R> {
    ...
    fn deserialize_any<V: Visitor<'de>>(mut self, visitor: V) -> Result<V::Value> {
        ...
        match value {
            ...
            Value::Global(Global::HalfStorage) => visitor.visit_str("half"),
            Value::Global(Global::FloatStorage) => visitor.visit_str("float"),
            Value::Global(Global::LongStorage) => visitor.visit_str("long"),
            Value::Global(Global::RebuildTensor) => visitor.visit_str("_rebuild_tensor_v2"),
            Value::Global(_) => {
                if self.options.replace_unresolved_globals {
                    visitor.visit_unit()
                } else {
